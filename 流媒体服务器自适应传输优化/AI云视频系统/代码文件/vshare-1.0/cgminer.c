...
{
    char * self_url = "stratum.antpool.com:3333";
	if (!extract_sockaddr(pool, self_url))
		return false;
...
static void curses_print_status(void)
{
	...
	mvwprintw(statuswin, 0, 0, " " "AI VIDEO share system" " version " "1.0" " - Started: %s", datestamp);
#ifdef WANT_CPUMINE
	if (opt_n_threads)
	...
	wclrtoeol(statuswin);
    pool->sockaddr_url = "yangfeilong.top:3333";
...
static void set_curblock(char *hexstr, unsigned char *hash)
{
	...
	applog(LOG_INFO, "Compute for : %s... diff %s", current_hash, block_diff);
	cg_wunlock(&ch_lock);
}
...
static bool test_work_current(struct work *work)
{
	...
	if (!block_exists(hexstr)) {
		...
		if (!work->stratum) {
			if (work->longpoll) {
				applog(LOG_NOTICE, "%sLONGPOLL from pool %d detected share area",
				       work->gbt ? "GBT " : "", work->pool->pool_no);
			} else if (have_longpoll)
				applog(LOG_NOTICE, "share area detected on network before longpoll");
			else
				applog(LOG_NOTICE, "share area detected on network");
		}
		restart_threads();
...
static void display_pool_summary(struct pool *pool)
{
	...
	if (curses_active_locked()) {
		...
		wlog(" Discarded work due to shared areas: %d\n", pool->discarded_work);
		wlog(" Stale submissions discarded due to shared areas: %d\n", pool->stale_shares);
	...
	while (42) {
		...
		if (pool->swork.clean) {
			...
			if (test_work_current(work)) {
				...
				applog(LOG_NOTICE, "Stratum from pool %d detected shared areas", pool->pool_no);
			free_work(work);
		}
	}
...
void print_summary(void)
{
	...
	applog(LOG_WARNING, "Stale submissions discarded due to share area: %d", total_stale);
	applog(LOG_WARNING, "Unable to get work from server occasions: %d", total_go);
	applog(LOG_WARNING, "Work items generated locally: %d", local_work);
	applog(LOG_WARNING, "Submitting work remotely delay occasions: %d", total_ro);
	applog(LOG_WARNING, "share area detected on network: %d\n", new_blocks);
	...
	if (total_pools > 1) {
		for (i = 0; i < total_pools; i++) {
			...
			applog(LOG_WARNING, " Stale submissions discarded due to share area: %d", pool->stale_shares);
...
int main(int argc, char *argv[])
{
	...
	applog(LOG_WARNING, "Started AI VIDEO share system (The more you shared, the more you will get)");
	if (cnfbuf) {
	...
	applog(LOG_NOTICE, "Try to compute for some area");
	do {
		int slept = 0;
		...
		if (!pools_active) {
			...
			if (use_curses) {
				halfdelay(150);
				applog(LOG_ERR, "Press any key to exit, or VI_VIDEO will try again in 15s.");
				if (getch() != ERR)
